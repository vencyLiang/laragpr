...
class UserController extends Controller{
    ...
    public function pay(User $user){
        ...
        if($result['status'] == 200){
            ...
            $lastTimeStamp = $user->account->{$currencySymbol."_address_time"};
            $time = GENERATE_ADDR_INTERVAL + $lastTimeStamp - time();
        }else{
            return jump('生成支付页面失败,请稍后重试',route('users.show',compact('user')),'个人中心');
        }
        $cost = ACTIVATE_COST;
    ...
    public function getArticles(Request $request, User $user){
        ...
        $data = $request->input();
        $perPage = $data['per'] ?? 10;
        $haveReadAidArr = $user->haveReadArticles->pluck('id');
        $result = Article::with(['admin'=>function($query){
            $query->select(['id','username','avatar']);
        }])->where('status','=',1)->whereNotIn('id',$haveReadAidArr)->orderBy('created_at','desc')->limit($perPage)->select(['id','admin_id','title','summary','created_at'])->get();
        //dd($result);
       // $user->haveReadArticles()->attach($result->pluck('id'));
        return Response::json(['count'=> $result->count() ,'data'=> $result->toArray()]);
    }
     ...
     * @throws \Illuminate\Auth\Access\AuthorizationException
     */
    public function setHaveRead(User $user){
        $this->authorize('update', $user);
        DB::beginTransaction();
        try{
            $user->haveReadArticles()->attach(json_decode(request()->input('id'),true));
            DB::commit();
        }catch (Exception $e){
            DB::rollBack();
            Response::json(['msg'=>$e->getMessage()]);
        }
    }
}
